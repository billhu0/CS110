...
void v_add_optimized_adjacent(double *x, double *y, double *z) {
    #pragma omp parallel
    {
        ...
        for (int i = current_thread; i < ARRAY_SIZE; i += total_threads) {
            z[i] = x[i] + y[i];
        }
    }
}
...
void v_add_optimized_chunks(double *x, double *y, double *z) {
    ...
    #pragma omp parallel
    {
        ...
        const int chunk_size    = ARRAY_SIZE / total_threads;
        const int rest_num      = ARRAY_SIZE - chunk_size * total_threads;
