...
// (without using OpenMP's builtin 'Reduction' keyword)
...
// (using the 'reduction' keyword, no more 'critical')
...
int main() {
    ...
    for (int i = 1; i <= num_threads; i++) {
        ...
        for (int j = 0; j < REPEAT; j++) result = dotp_manual_optimized(x, y, array_size);
    ...
    for (int i = 1; i <= num_threads; i++) {
        ...
        for (int j = 0; j < REPEAT; j++) result = dotp_reduction_optimized(x, y, array_size);
