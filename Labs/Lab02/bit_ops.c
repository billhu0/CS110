...
unsigned get_bit(unsigned x, unsigned n) {
    /* To get the bit, simply move the bit to 
        the least significant bit, 
        and then output it by 'and'ing 0x0001 */
    return (x >> n) & 1;
}
...
void set_bit(unsigned *x, unsigned n, unsigned v) {
    /* First, generate a number like 1111011111
        and then taking the 'and'. 
        This FORCES the bit TO ZERO. */
    unsigned int tmp_set2zero = ~(1 << n);
    *x &= tmp_set2zero;
    /* Then, creating a number like 0000v00000,
        and then taking the 'or'.
        This sets the correspoing bit to V. */
    unsigned int tmp_set2v = v << n;
    *x |= tmp_set2v;
    /* And we are done! */
    return;
}
...
void flip_bit(unsigned *x, unsigned n) {
    /* With the help of the above two functions, 
        to flip a bit, we just need to
        get it, inverse it, and set it! */
